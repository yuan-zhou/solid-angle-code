...
def check_sign_consistency(A):
    ...
    in particular, the matrix is not positive definite.::
    ...
    .. NOTE::
        ...
        solid angle that can be computed using Ribando's power series.
    """
    n = A.nrows()
    s = [0]*n
    for i in range(n):
...
def solid_angle_measure(A, eps=1e-6, deg=100, simplicial=False):
    ...
    .. NOTE::
        ...
        2.2. The power series converges to the normalized solid angle
        measure if and only if the associated matrix to the cone is positive
        definite. This function decomposes the cone into cones with positive
        definite associated matrices, hence into cones whose solid angles
        are computable via Ribando's formula.
    """
